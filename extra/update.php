...
class DownloadUpdate extends Command {
    ...
    public function fire()
    {
        ...
        $mainRegexes = file_get_contents($this->getOption('source'));
        ...
        if($patchFile = $this->getOption('patch'))
        {
            ...
            $mainRegexes .= $this->loadPatch($patchFile);
        }
        
        // Translate the nomenclature into the correct names for our parser
        $str = strtr($mainRegexes, $this->translations);
        ...
        $this->write($this->getOption('output'), $final, $interaction);
        
        $capabilities = spyc_load(file_get_contents($this->getOption('capabilities')));
        
        $this->write($this->getOption('outputcapabilities'), $capabilities, $interaction);
    ...
    public function loadPatch($patchFile)
    {
        if(is_dir($patchFile))
        {
            ...
            foreach(new IteratorIterator(new RecursiveDirectoryIterator($patchFile)) as $file)
            {
                $str .= $this->loadPatch($file);
            }
            
            return $str;
        }
        
        if(is_file($patchFile) && substr($patchFile, -5) === '.yaml')
        {
    ...
    {
        return array(
            array('source','s', self::OPTIONAL, 'https://raw.github.com/tobie/ua-parser/master/regexes.yaml'),
            array('capabilities','c', self::OPTIONAL, 'https://raw.github.com/joegreen0991/ua-parser/master/resources/capabilities.yaml'),
            array('output','o', self::OPTIONAL, __DIR__ . '/../resources/regexes.php'),
            array('outputcapabilities','p', self::OPTIONAL, __DIR__ . '/../resources/capabilities.php'),
            array('patch','p', self::OPTIONAL),
